...
CVSoutput_dir = '/home/gcng/workspace/ProjectFiles/AndesWaterResources/GSFLOW/outputs/';
...
con_filname = strcat(PRMSinput_dir, inname, '/gsflow.control');
...
%     temp_module = 'temp_1sta';
    ...
    et_module = 'potet_pt'; % use default alpha = 1.26)
...
humidity_datafil = strcat(PRMSinput_dir, inname, '/humidity.day');
transp_datafil = strcat(PRMSinput_dir, inname, '/transp.day'); % may not be needed in GSFLOW? Is needed!

fl_write_climate = 0;  % 1: run just to create climate_hru files; 0: full GSFLOW
...
con_par_type(ii) = 4; 
if fl_write_climate
    con_par_values{ii} = {'WRITE_CLIMATE'}; % to generate climate_hru files
else
    con_par_values{ii} = {'GSFLOW'}; % to run coupled model
end

ii = ii+1;
con_par_name{ii} = 'cvs_output_file';
con_par_type(ii) = 4; 
con_par_values{ii} = {[outdir, 'gsflow.cvs']};
...
con_par_type(ii) = 4; 
con_par_values{ii} = {[outdir, 'modflow.nam']};
...
ii = ii+1;
con_par_name{ii} = 'gsf_rpt'; % flag to create cvs output file
con_par_type(ii) = 4; 
con_par_values{ii} = 1;
...
ii = ii+1;
con_par_name{ii} = 'cbh_binary_flag'; 
con_par_type(ii) = 1; 
con_par_values{ii} = 0; % 0 for no, use default

%Read a CBH file with humidity data
ii = ii+1;
con_par_name{ii} = 'humidity_cbh_flag'; 
con_par_type(ii) = 1; 
con_par_values{ii} = 0; % 0 for no, use default

%Variable orad
ii = ii+1;
con_par_name{ii} = 'orad_flag'; 
con_par_type(ii) = 1; 
con_par_values{ii} = 0; % 0 for no, use default
