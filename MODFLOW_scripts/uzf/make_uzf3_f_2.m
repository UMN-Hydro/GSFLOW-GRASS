function make_uzf3_f_2(GSFLOW_indir, infile_pre, surfz_fil, mask_fil)
...
% - set FINF (for S.S. initialization) according to TOP (high elev only)
...
TOP_mask = TOP; TOP_mask(IBOUND==0) = 0;
NZ = length(find(TOP_mask));
z_sort = sort(TOP_mask(:), 'descend');

% this works: (for no sfr: converges, but mostly all dry)
ind = TOP_mask > z_sort(round(NZ/10));
finf = zeros(NROW,NCOL);
...
finf = zeros(NROW,NCOL);
ind = TOP_mask > z_sort(round(0.25*NZ));
finf(IBOUND~=0) = 4e-4 / 10; % m/d (8.8e-4 m/d typical in Sagehen)
finf(ind) = 4e-4; % m/d (8.8e-4 m/d typical in Sagehen)

% scale FINF by elev
% (% Sagehen max 3*8.e-4, min 8.e-4
maxFINF = 3*8.e-4; minFINF = 8e-4; 
midZ_factor = (2150-1928)/(2649-1928); % based on Sagehen, where FINF levels off; sagehen: 0.31
midZ = midZ_factor * (max(TOP_mask(:))-min(TOP_mask(TOP_mask>0))) + min(TOP_mask(TOP_mask>0));
m = (maxFINF-minFINF)/(max(TOP_mask(:))-min(TOP_mask(TOP_mask>0)));
finf = m*(TOP_mask-min(TOP_mask(TOP_mask>0))) + minFINF;
