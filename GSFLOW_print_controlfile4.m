...
datafil = strcat(PRMSinput_dir, 'empty.day');
...
namfil = strcat(MODFLOWinput_dir, 'test2lay.nam');
...
precip_datafil = strcat(PRMSinput_dir, 'precip.day');
tmax_datafil = strcat(PRMSinput_dir, 'tmax.day'); % to be safe: set as F
tmin_datafil = strcat(PRMSinput_dir, 'tmin.day'); % to be safe: set as F
solrad_datafil = strcat(PRMSinput_dir, 'swrad.day');
pet_datafil = strcat(PRMSinput_dir, 'potet.day');
% humidity_datafil = strcat(PRMSinput_dir, 'humidity.day'); % for potet_pm
transp_datafil = strcat(PRMSinput_dir, 'transp.day'); % may not be needed in GSFLOW? Is needed!
...
cmd_str_cmt = ['#', GSFLOW_exe_cmt, ' ', con_filname];
cmd_str = [GSFLOW_exe, ' ', con_filname];
